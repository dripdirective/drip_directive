...
const OutfitCard = ({ occasion, match, items, isSecond }) => (
    <View style={styles.card}>
        <View style={styles.cardHeader}>
            <View style={[styles.tag, { backgroundColor: isSecond ? '#fce7f3' : '#e0e7ff' }]}>
                <Text style={[styles.tagText, { color: isSecond ? '#be185d' : '#4338ca' }]}>{occasion}</Text>
            </View>
        ...
        {isSecond ? (
            <TouchableOpacity style={[styles.wearBtn, styles.shopBtn]} activeOpacity={0.9}>
                <Text style={[styles.wearBtnText, styles.shopBtnText]}>Shop Missing Piece</Text>
            </TouchableOpacity>
        ) : (
            <TouchableOpacity style={styles.wearBtn} activeOpacity={0.9}>
...
export default function VibeSyncOutfitsScreen() {
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                <OutfitCard occasion="Work" match={85} items={['🧥', '👔', '👖', '👞']} />

                <OutfitCard occasion="Date Night" match={78} items={['🧥', '👕', '👖', '👟']} isSecond />
            </ScrollView>
        </View>
    );
}
