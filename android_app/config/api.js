...
const getApiBaseUrl = () => {
  ...
  // Expo: set `EXPO_PUBLIC_API_BASE_URL` in `android_app/.env`
  if (process.env.EXPO_PUBLIC_API_BASE_URL) {
    return process.env.EXPO_PUBLIC_API_BASE_URL;
  }

  // 2. Use production API for production builds (fallback)
  if (process.env.NODE_ENV === 'production') {
    return 'https://api.dripdirective.com';
  }

  // 3. Use localhost for development (fallback)
  // If testing on a physical device, set EXPO_PUBLIC_API_BASE_URL to your LAN IP.
